...
export const useUsers = () => {
  ...
  const assignStaff = async (id: string) => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/account/assign/staff/${id}`, {
        method: "POST",
        headers: {
      ...
      console.warn("API not available for staff assignment:", err);
      // Update role in mock data locally
      setUsers((prev) => prev.map((user) => (user.id === id ? { ...user, role: "ROLE_STAFF" } : user)));
    }
  };

  const unassignStaff = async (id: string) => {
    try {
      const token = localStorage.getItem("admin_token");
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/account/unassign/staff/${id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      console.warn("API not available for staff unassignment:", err);
      // Update role in mock data locally
      setUsers((prev) => prev.map((user) => (user.id === id ? { ...user, role: "ROLE_USER" } : user)));
    }
  };
  ...
  const addCredit = async (id: string, amount: number) => {
    try {
      ...
      const requestJson = {
        userId: id,
        description: "Thêm credit từ quản trị viên",
        money: amount,
        type: "add",
      };
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/Vnpay/save-transaction`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      console.warn("API not available for credit addition:", err);
      // For mock data, we could add a credit field, but since it's not in the User interface, we'll just return success
      console.log(`Mock: Added ${amount} credit to user ${id}`);
      return { success: true, message: `Added ${amount} credit` };
    }
  };
